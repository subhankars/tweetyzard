...
using tweetyzard.winform.LocalResources;
...
namespace tweetyzard.winform
{
    public static class OpenXMLHelper
    {
        ...
        private static void DeleteWorkSheet(WorkbookPart wbPart, string sheetToDelete)
        {
            ...
            var sheet = wbPart.Workbook.Descendants<Sheet>().FirstOrDefault(s => s.Name == sheetToDelete);
            if (sheet == null)
            {
                return;
            }
            ...
            string currentSheetId = sheet.SheetId;
            ...
            {
                var calChainEntries = calChainPart.CalculationChain.Descendants<CalculationCell>().Where(c => c.SheetId == currentSheetId);
                var calcsToDelete = calChainEntries.ToList();
